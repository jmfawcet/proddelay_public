...
transformed parameters {
  ...
  matrix[S, N_g-1] theta; // prob. of recollection (logit link)
  array[S] vector[K-1] thresh;
  
  for (s in 1:S) {
    dprime[s] = mu' + u_dprime[s];
    theta[s] = inv_logit(mu_theta' + u_theta[s]);
...
model {
  ...
  for (n in 1:N) {
    ...
    if (is_old[n]) {
      target += log_mix(
        theta[s, group[n]], 
        (y[n] == K) ? 0 : negative_infinity(), // if recollection occurs, always respond with max confidence
        ordered_probit_lpmf(y[n] | dprime[s, group[n]], thresh[s]) // if not, regular EVSDT likelihood
      );
    } else {
