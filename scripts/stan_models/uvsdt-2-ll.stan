...
transformed parameters {
  ...
  for (s in 1:S) {
    ...
    theta[s] = mu_theta[,exp_idx]' + u_theta[s];
...
model {
  ...
  for (n in 1:N) {
    ...
    if (is_old[n] && g < N_g) {
      target += uv_sdt_lpmf(y[n] | dprime[s,g], exp(theta[s,g]), thresh[s]);
    } else {
...
generated quantities {
  ...
  for (n in 1:N) {
    ...
    if (is_old[n] && g < N_g) {
      log_lik[n] = uv_sdt_lpmf(y[n] | dprime[s,g], exp(theta[s,g]), thresh[s]);
    } else {
