...
parameters {
  vector[N_g-1] mu;
  vector[N_g-1] mu_theta; // 1 - prob. of guessing - logit scale
  real beta_thresh1;
...
model {
  ...
  for (n in 1:N) {
    ...
    if (is_old[n]) {
      target += log_mix( // if items are old, use a mix of the old and new ordered probit likelihoods
        theta[s, group[n]],
        ordered_probit_lpmf(y[n] | 0, thresh[s]),
